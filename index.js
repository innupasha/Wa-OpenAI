...
const sessionName = 'innupasha'
const donet = 'https://saweria.co/innupasha'
const owner = ['6281277777003']
const { default: innupashaConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto, getContentType } = require("@adiwajshing/baileys")
...
async function startHisoka() {
    ...
    const client = innupashaConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Wa-OpenAI - InNupasha','Safari','3.0'],
        auth: state
    })
    ...
    client.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./innupasha")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    client.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Delete Session file innupasha.json and Scan Again.`); process.exit(); }
